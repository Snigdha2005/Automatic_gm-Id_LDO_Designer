...
ASC_FILE = r"C:\Users\SnigdhaYS\Documents\LTSpice_LDO_Automation\Internally_Compensated\Miller_LDO_Sim_Benches_502\LDO_loopgain_IIIT.cir"
LTSPICE_PATH = r"C:\Program Files\LTC\LTspiceXVII\XVIIx64.exe"
PSRR_ASC_FILE = r"C:\Users\SnigdhaYS\Documents\LTSpice_LDO_Automation\Internally_Compensated\Miller_LDO_Sim_Benches_502\LDO_PSRR_IIIT.cir"
TRANS_ASC_FILE = r"C:\Users\SnigdhaYS\Documents\LTSpice_LDO_Automation\Internally_Compensated\Miller_LDO_Sim_Benches_502\LDO_Transient_IIIT.cir"
LOG_FILE = r"C:\Users\SnigdhaYS\Documents\LTSpice_LDO_Automation\Internally_Compensated\Miller_LDO_Sim_Benches_502\LDO_loopgain_IIIT.log"
...
def run_temperature_sweep(spec_file_name, cir_file_path, ltspice_exe, params, temps=[-40, 27, 125]):
    ...
    for i in temps:
        ...
        l1 = ltspice.Ltspice("C:/Users/SnigdhaYS/Documents/LTSpice_LDO_Automation/Internally_Compensated/Miller_LDO_Sim_Benches_502/LDO_loopgain_IIIT.op.raw")
        l1.parse()
...
def analyze_loopgain(raw_file, fp1_theo, loop_gain_theo):
    ...
    #print(f"Low-frequency loop gain: {low_mag_db:.2f} (theo: {loop_gain_theo:.2f}) -> Error: {loop_gain_error:.2f} %")
...
# Load Specs
# -----------------------------
def run_lt_spice_internal(spec_file_name, gm_id_best):
    ...
    cgs_cgd = gm  * (10**6)/ (2 * np.pi * ft)
    # #print(cgs_cgd)
    Ilload = float(spec["Iload|min"])
    id_w = Ilload * 1000 / W
    gm_id_from_idw_fun = interp1d(idw_vals, gm_id_vals, fill_value="extrapolate")
    gm_id = float(gm_id_from_idw_fun(id_w))
    gmro_fun = interp1d(gm_id_vals, gmro_vals, fill_value="extrapolate")
    gm_ro = float(gmro_fun(gm_id))
    gm_light = gm_id * Ilload / 1000
    wp2_heavy = gm * 10**6 / (spec["Cload"]+cgs_cgd/2)
    wp2_light = gm_light * 10**6 / spec["Cload"]
    # #print("wp2 light: ", wp2_light)
    fp2_light = wp2_light / (2*np.pi)
    wp1 = wp2_light / (spec["loop_gain"]*gm_ro/gmro)
    # #print(wp1)
    fp1 = wp1 / (2 * np.pi)
    ...
    #print("cgs_cgd               =", cgs_cgd)
    #print("id_w = ", id_w)
    #print("gm_id = ", gm_id)
    #print("gmro = ", gm_ro)
    #print("low freq gain = ", spec["loop_gain"] * gm_ro / gmro)
    #print("gm_light = ", gm_light)
    #print("================================================")
    ...
    Id = Iq / 2
    gm_nmos = gm_id_target * Id / 1000000
    ...
    Id_load = Iq / 2
    gm_pmos = gm_id_target * Id_load / 1000000
    ...
    if os.path.exists(raw_file):
        os.remove(raw_file)
    # raw_file = run_ltspice(updated_asc)
    ...
    # Update the schematic with same params
    ...
    params["Iload"] = f"{spec['Iload|max']}m"
    params["iload_min"] = f"{spec['Iload|min']}m"
    params["tdelay"] = f"{spec['tdelay']}m"
    params["trise"] = f"{spec['trise']}m"
    params["tfall"] = f"{spec['tfall']}m"
    params["ton"] = f"{spec['ton']}m"
    params["tperiod"] = f"{spec['tperiod']}m"
    params["ncycles"] = f"{spec['ncycles']}m"
    ...
    # plt.tight_layout()
    # plt.show()
# run_lt_spice_internal("specs/spec3.xlsx", 17.563025210084035)