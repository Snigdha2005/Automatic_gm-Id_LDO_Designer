...
def all_in_saturation(op_file):
    ...
    for dev, vals in op_values.items():
        try:
            ...
            Vds = vals['Vds']
            Vth = vals['Vth']
            if not (abs(Vds) > abs(Vgs) - abs(Vth)):
                all_sat = False
        except KeyError:
            all_sat = False
...
# Load Specs
# -----------------------------
def best_gm_id_external_bandwidth(spec):
    ...
    for gm_id_target in gm_id_vals:
        ...
        if total_error < min_error and total_error > 0 and phase_margin_sim >= 45 and fmax < fp1_sim:
